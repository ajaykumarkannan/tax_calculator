...
def compute_tax(country: str, state: str, income: float, year: int = 2022) -> float:
    ...
    total_taxes = max(
        0,
        compute_tax_helper(income, state_data)
        - compute_BPA(country, state, income, year),
    )
...
def plotComparisons(
    input_list=None, income_bound=300000, plotIncome=True, plotRate=False
):
    db = load_database()
